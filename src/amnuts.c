...
main(int argc, char **argv)
{
    ...
    init_signals();
    create_system();
    yaml_check_migration_gate();
    yaml_load_config(DATAFILES "/config.yaml");
    yaml_load_rooms(DATAFILES "/rooms.yaml");
    yaml_load_help(DATAFILES "/help.yaml");
    validate_config();
...
/*
 * Cross-cutting validation run after the YAML config + rooms have been
 * loaded but before help is loaded. Checks port presence and uniqueness,
 * rejects an empty room list, and reconciles netlink service names with
 * room names. Loaders themselves enforce field presence within their own
 * files; this function handles checks that span both.
 */
void
validate_config(void)
{
...
#ifdef NETLINKS
    /* Service names must not collide with room names */
    for (nl = nl_first; nl; nl = nl->next) {
    ...
    /* Resolve room netlink_name to netlink object */
    for (rm1 = room_first; rm1; rm1 = rm1->next) {
        if (!*rm1->netlink_name) {
            continue;
        }
